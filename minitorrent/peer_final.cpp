...
map<string,vector<int>> piece_info;//contains piece info for a file. Key is filename and value is pieces this client have (like xyz 12345) so xyz is filename and 1 is chunkname then 2 and so on...
pthread_mutex_t lock1; //lock declairation 
pthread_mutex_t lock2; 
pthread_mutex_t lock3; 
struct sr
{
	vector<string> ip_port; ///0th contain ip, 1st contain port and then indexes.
	FILE*fp;
	string filename;
	string file_size;
	int port;
   ...
   //sending input command to tracker.------------------------------------------------------------------------------------
    ...
    if(inputs.size()>=3)
    {
		if(inputs[2]=="upload_file")
		{
			...
			{
			    piece_info[inputs[4]].push_back(i);  //v2 inputs[4] is filename and inputs[5] is size	
			}
		}
    }
...
{  
	...
	int prt=1029; //v2 this port is used to send the requests to tracker as it is tracker port number
	
	struct sockaddr_in server_add;
	server_add.sin_family=AF_INET;
	server_add.sin_addr.s_addr = INADDR_ANY; 
	server_add.sin_port=htons(prt);
	server_add.sin_addr.s_addr=inet_addr("127.0.0.1"); // v2 this is tracker ip 
	// upto here---------------------------------------------------------------------------------------------------
    ...
    while(1)//for taking input infinitely from  peer terminal.
    {
       ...
       pthread_mutex_lock(&lock2); 
		while(cin>>str && str!="q")  //v2 replace q with newline as this is terminating character
		{  
...
{   
    ...
    // cout<<"hello12"<<endl;
	if(inputs[0]=="make_file")   //v2 file info recieving from tracker and we can see that at line number 523 and 525
	{
	...
	else if(inputs[0]=="send_pieces")
	{
			...
			for(int i=0;i<num_of_indexes;i++)//sending chunks of file
		{
			...
			cout<<"..................printing to find bug in send piece msg reader.............................."<<endl;
			n=fread(buffer,1,sizeof(buffer),fp1);

			cout<<(indexes[i])<<"chunk_no"<<endl;	
			cout<<"chunk_size_read"<<n<<endl;



			cout<<".....................bug printer ends here......................................."<<endl;
	...
	else if(inputs[2]=="peer_authentication")//third index is command in this case.cmd(ip,port,cmd,groupid).
	{
		...
		cout<<"Please Enter,:-  accept q   ";
		cout<<" and then Enter 1 or 2 accordingly"<<endl;
		int p;
		// cin.tie(nullptr);
		
		// when this command goes to request handler node of a group,then race condition happens between its command handler
		// and this request, and so command handler gets first so we need to give a command that is written above.Now as soon as you
		// give that command the lock2 unlocks and then request handler locks it and then you can press 1 or 2 and accordingly 
		// and then it again unlocks and then again command handler locks the lock2.
